...
function renderCandidates(parent,candidates,onSelect,targetRef="candidate",kind="equipment_part"){ const heading=document.createElement("strong"); heading.textContent="CANDIDATES"; parent.append(heading);
  ...
  const group=`candidate-${crypto.randomUUID()}`; candidates.forEach(candidate=>{ const row=document.createElement("label"); row.className="engineering-candidate"; const radio=document.createElement("input"); radio.type="radio"; radio.name=group;
    ...
    radio.onchange=()=>{ text.dataset.state="confirmed-draft"; text.textContent=`REVIEWER DECISION: use ${candidate.canonical_id}`; engineeringWorkflow.decisions.set(targetRef,{target_ref:targetRef,kind,action:"use_existing",canonical_id:candidate.canonical_id,expected_canonical_version:String(candidate.metadata.active_version??candidate.metadata.version??candidate.metadata.row_version??"")||null}); if(onSelect)onSelect(candidate.canonical_id); }; row.append(radio,text); parent.append(row); });
...
function renderConfirmedCommands(){const section=engineeringSection(engineeringReviewDraft,"CONFIRMED COMMANDS — READY / NOT EXECUTED");engineeringWorkflow.commands.forEach(command=>{const row=document.createElement("div");row.className="engineering-review-row";row.textContent=`${command.command_id} · ${command.command_type} · ${command.status.toUpperCase()} · ${JSON.stringify(command.payload)}`;section.append(row);});engineeringReviewDraft.querySelectorAll("input,button").forEach(control=>control.disabled=true);const warning=document.createElement("p");warning.textContent="Confirmed does NOT mean written to OpcTagManager.";section.append(warning);}
